...
token_t c_parse_exprs_or_type(tokenizer_t *tkn_ctx) {
    // TODO: Types support goes here.
    return c_parse_exprs(tkn_ctx);
}
    ...
    while (1) {
        ...
        if (is_tkn(0, C_TKN_LBRAC)) {
            ...
            if (is_operand(1) && peek.type == TOKENTYPE_OTHER && peek.subtype == C_TKN_RPAR) {
                ...
                pop();
                push(c_parse_exprs_or_type(tkn_ctx));
            }
            token_t tmp = tkn_peek(tkn_ctx);
        ...
        } else if (is_operand(1) && stack[stack_len - 1].type == TOKENTYPE_AST
        ...
        } else if (is_operand(1) && (is_tkn(0, C_TKN_INC) || is_tkn(0, C_TKN_DEC))) {
            // Reduce suffix.
            token_t op  = pop();
            token_t val = pop();
