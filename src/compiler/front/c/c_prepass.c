...
void c_prepass_expr(c_prepass_t *prepass, c_prescope_t *scope, token_t const *expr, bool is_addrof) {
...
void c_prepass_stmt(c_prepass_t *prepass, c_prescope_t *scope, token_t const *stmt) {
...
void c_prepass_decls(c_prepass_t *prepass, c_prescope_t *scope, token_t const *decls) {
    for (size_t i = 1; i < decls->params_len; i++) {
        ...
        if (decl->type == TOKENTYPE_AST && decl->subtype == C_AST_ASSIGN_DECL) {
            c_prepass_decl(prepass, scope, &decl->params[0]);
            c_prepass_expr(prepass, scope, &decl->params[1], false);
        } else {
            c_prepass_decl(prepass, scope, decl);
        }
    }
}

// Pre-compilation pass for a single direct (abstract) declarator.
void c_prepass_decl(c_prepass_t *prepass, c_prescope_t *scope, token_t const *decl) {
    (void)prepass;
    (void)scope;
    while (1) {
